...
- [`fsa-lfp-eligibility-events.json`](https://data.sustainable-fsa.com/fsa-lfp-eligibility/fsa-lfp-eligibility-events.json)
...
(`fsa-lfp-eligibility-events.csv`, `fsa-lfp-eligibility-events.parquet`,
and `fsa-lfp-eligibility-events.json`), the QA report (`qa-report.txt`),
...
- **Column-oriented**: one array per field with one entry per record,
  ...
  so the file gzips from ~2.9 MB to roughly 280 KB over the wire.
- **Dictionary-coded strings**: pasture types, FSA county codes, Census
  ...
  array; the record arrays hold integer indices into them.
- **Compact dates**: each `Qualifying Date` is a day-of-year (`qy`) plus
  ...
  carries qualifying dates there too — 13% of dated events here — so the
...
keys*](#🗺️-two-county-keys)). Nulls in `qy` and `qo` are the 2008–2011
duration tiers with no recoverable satisfaction date; nulls in `mepm`
are the 2008–2011 records, whose FOIA response carries no
`Maximum Eligible Payment Months` at all (see *Program years
2008–2011*).
