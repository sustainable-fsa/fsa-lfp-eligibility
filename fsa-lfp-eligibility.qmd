...
// Reactive card dimensions — uses #map (created by Quarto from label: map) to
// find the card body. Sizing the plot from a ResizeObserver makes the FIRST
// render fill the card; the stdlib `width` is only re-evaluated on window
// resize, which left the initial paint at the wrong scale.
...
height: Math.max(300, cardSize.height - 70),  // leave room for the legend
projection: {
    type: "albers",
    domain: counties
  },
